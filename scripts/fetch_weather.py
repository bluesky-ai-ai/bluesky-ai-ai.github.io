...
def scrape() -> dict:
    ...
    # ── 습도 및 바람 (summary_list 에서 파싱 시도) ──────────────
    for div in soup.select(".summary_list .sort"):
        term = div.select_one(".term")
        desc = div.select_one(".desc")
        if term and desc:
            ...
            elif "풍" in term_text or "바람" in term_text:
                data["wind"] = desc_text
